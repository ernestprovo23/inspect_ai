...
async def hydrate(
    *,
    ...
    if is_remote_destination(new_sample_checkpoints_dir):
        ...
        _debug(f"[hydrate] sample_staging_dir={sample_staging}")
    else:
        sample_staging = None
        sample_root = new_sample_checkpoints_dir
    _debug(f"[hydrate] sample_root={sample_root}")

    sample_context_dir = await ensure_context_dir(sample_root)
    _debug(f"[hydrate] context_dir={sample_context_dir}")
    ...
    if resume_checkpoint:
        ...
        _debug(
            f"[hydrate.copy.cross-cutting] {resume_checkpoint.sample_checkpoints_dir}"
            f" -> {sample_root}"
        )
        copied = await _fs_copy_cross_cutting(
            resume_checkpoint.sample_checkpoints_dir,
            sample_root,
        )
        _debug(f"[hydrate.copy.cross-cutting] copied ({len(copied)}): {copied}")
    restic_config = await ensure_restic_config(sample_root)
    _debug(f"[hydrate] restic_password={restic_config.restic_password[:8]}...")
    ...
    if resume_checkpoint:
        ...
        _debug(f"[hydrate] latest committed checkpoint id: {latest_committed_id}")
    ...
    if resume_checkpoint and sample_staging is not None:
        count = await anyio.to_thread.run_sync(seed_manifest, sample_staging)
        _debug(f"[hydrate] seeded egress manifest at {sample_staging}: {count} entries")
...
async def _hydrate_host(
    *,
    ...
    _debug(
        f"[hydrate.host] resume: FS-copy {resume.sample_checkpoints_dir}/restic/host"
        f" -> {host_repo}"
    )
    await _fs_copy_repo(
    ...
    _debug(f"[hydrate.host] restic restore latest -> {context_dir}")
...
async def _hydrate_sandbox(
    *,
    ...
    _debug(
        ...
        f" {resume.sample_checkpoints_dir}/restic/sandboxes/{name} -> {new_host_side_repo}"
    )
    await _fs_copy_repo(
        resume.sample_checkpoints_dir,
...
async def _fs_copy_cross_cutting(old_sample_dir: str, new_sample_dir: str) -> list[str]:
    ...
    if await async_fs.exists(src_restic_config):
        ...
        _debug(f"[hydrate.copy] restic-config.json: {src_restic_config} -> {dst}")
    ...
    async for uri in async_fs.iter_files(old_sample_dir, pattern="ckpt-*.json"):
        ...
        written.append(name)
        count += 1
    _debug(f"[hydrate.copy] checkpoint files copied: {count}")
    return written
...
async def _fs_copy_repo(
    ...
    async for uri in async_fs.iter_files(src_base, recursive=True):
        rel = uri[len(src_base) + 1 :]  # strip "<src_base>/"
        dst = new_root / rel
    ...
    _debug(
        f"[hydrate.copy] {label} repo: {src_base} -> {new_repo} ({len(written)} files)"
    )
    return written
