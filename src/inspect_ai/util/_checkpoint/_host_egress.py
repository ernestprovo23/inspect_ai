...
def seed_manifest(staging_dir: str) -> int:
    ...
    _debug(f"[seed_manifest] {staging_dir}: {len(files)} entries: {sorted(files)}")
    return len(files)
...
async def host_egress(*, staging_dir: str, destination_dir: str) -> None:
    ...
    _debug(
        f"[host_egress] {staging_dir} -> {destination_dir} : "
        f"manifest={len(shipped)} entries, new={len(new_files)} files"
    )
    if not new_files:
        _debug("[host_egress] no new files — skip")
        return
    ordered = _safe_order(new_files)
    _debug(f"[host_egress] shipping (ordered, n={len(ordered)}): {ordered}")
    ...
    _debug(
        f"[host_egress] done; manifest now has {len(shipped) + len(ordered)} entries"
    )
